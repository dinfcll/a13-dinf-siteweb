...
(function () {
    ...
    var O = function () {
        ...
        a.prototype = {show: function () {
            ...
            var g = this, d = g.el, b = g.options, c = g.calendar, j = "gldp-" + b.cssName, t = b.todayDate._val(), u = t.time, h = b.borderSize + "px", y = function (b, m, a) {
                ...
                y = y(0, 6, b.selectableDOW), z = b.dowNames || "Dim Lun Mar Mer Jeu Ven Sam".split(" "), t = b.monthNames || "Janvier Février Mars Avril Mai Juin Juillet Aout Septembre Octobre Novembre Décembre".split(" "), f = d.outerWidth(), e, k = f / 7 + 6 * (b.borderSize / 7), f = f / 8 + 7 * (b.borderSize / 8);
            c.length ? eval(c.data("is")) || (f = c.outerWidth(), e = c.outerHeight(), k = f / 7 + 6 * (b.borderSize / 7), f = e / 8 + 7 * (b.borderSize / 8)) : (g.calendar = c = $("<div/>").attr("gldp-el", d.attr("gldp-id")).data("is", !0).css({display: b.showAlways ? void 0 : "none", zIndex: b.zIndex,
                width: 7 * k + "px"}), $("body").append(c));
            d.is(":visible") || c.hide();
            c.removeClass().addClass(j).children().remove();
            j = function () {
                var i = d.offset();
                c.css({top: i.top + d.outerHeight() + b.calendarOffset.y + "px", left: i.left + b.calendarOffset.x + "px"})
            };
            $(window).resize(j);
            j();
            ...
            k = $("<div/>").addClass(" core border monyear title").css($.extend({}, j, {width: k + "px", borderTopWidth: h, marginLeft: "-" + h}));
            l = $("<div/>").addClass(" core border monyear ").css($.extend({}, j, {marginLeft: "-" + h, borderWidth: h + " " + h + " 0 0"})).append($("<a/>").addClass("next-arrow" + (B ? "" : "-off")).html(b.nextArrow)).mousedown(function () {
                return!1
            }).click(function (i) {
            ...
            for (l = e = 0; 7 > e; e++)for (var q = 0; 7 > q; q++, l++) {
                ...
                if (e) {
                    ...
                    !s || 0 > v._indexOf(p.year) || 0 > H._indexOf(p.month) || 0 > y._indexOf(p.day) ? n = "noday" : (n = "Dim Lun Mar Mer Jeu Ven Sam".split(" ")[p.day], I != p.month && (n += " outday"), u == x && (n = "today", r += 50), b.selectedDate._time() == x && (n = "selected", r += 51), b.specialDates && $.each(b.specialDates, function (b, a) {
                        D(a, a.date).time ==
                ...
                $.extend(j, {borderTopWidth: h, borderBottomWidth: h, borderLeftWidth: 0 < e || !e && !q ? h : 0, borderRightWidth: 0 < e ||
                    !e && 6 == q ? h : 0, marginLeft: 0 < q ? "-" + h : 0, marginTop: 0 < e ? "-" + h : 0, zIndex: r});
    ...
    Date.prototype._val = function () {
        this._clear();
        return{year: this.getFullYear(), month: this.getMonth(), date: this.getDate(), time: this.getTime(), day: this.getDay()}
    };
